...
class ObstacleAvoidanceModel:
    ...
    # def get_near_tangent(self,yaw1:Tuple[float] , yaw2:Tuple[float], m1, m2):
    #     l1,_,r1 = yaw1
    #     l2,_,r2 = yaw2

    #     cnt = m1+m2/2
    #     yaw_cnt = np.arctan2(cnt[1],cnt[0])
    #     diff_l1 = ((l1%(2*np.pi))-yaw_cnt)%(2*np.pi)
    #     diff_r1 = ((r1%(2*np.pi))-yaw_cnt)%(2*np.pi)
    #     diff_l2 = ((l2%(2*np.pi))-yaw_cnt)%(2*np.pi)
    #     diff_r2 = ((r2%(2*np.pi))-yaw_cnt)%(2*np.pi)

    #     idx = np.array([diff_l1,diff_r1,diff_l2,diff_r2]).argsort()[0,-1]
    #     return idx            
        ...
        if len(attention) == 0:
            ...
            if len(attention) == 1:
                ...
                if np.sign(np.cross(ve,tangets[0][0])) != np.sign(np.cross(ve,tangets[0][2])):
                    ...
                    obs[0] += uav.x ; obs[1]+=uav.y
                    theta = np.arctan2(obs[1] - uav.y, obs[0] - uav.x)
                    vo = np.linalg.norm(ve) * np.array([np.cos(theta),np.sin(theta)]) *weight
                else:
                    vo = ve.copy()
            else:
    ...
    def get_Ind_2(self, uav: UAV, ind1_idx:int , obstacles: List[Obstacle], yaws:List[float], tangents:List[Tuple[np.ndarray,np.ndarray,np.ndarray]]) -> List[int]:
        ...
        for idx,obstacle in enumerate(obstacles):
            ...
            if np.cross(c1,tangents[idx][1]) > 0:
                ...
                t1 = t11 if np.cross(c1,t11) < 0 else t12
                t2 = tangents[idx][0] if np.cross(tangents[idx][1],tangents[idx][0]) > 0 else tangents[idx][2]
            gap = c1.dot(t2)/(np.linalg.norm(t1)*np.linalg.norm(t2)+1e-6)
            if gap > mgap:
                mgap = gap
                midx = idx
        return midx
...
class FlockingControlAlgorithm:
    ...
    def render_frame(self, t, vf_dots=None, vos=None):
        ...
        for i, uav in enumerate(self.uavs[:1]):
