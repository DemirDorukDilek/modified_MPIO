...
class ObstacleAvoidanceModel:
    ...
    def calculate_force(self, uav: UAV, obstacles: List[Obstacle], ve: np.ndarray, weight:int):
        ...
        if system.uavs.index(uav) == 1:
            # print("t:",system.TT,[obstacles.index(x)+1 for x in attention])
            self.gp.clear()    
        ...
        if len(attention) == 0:
            ...
            if len(attention) == 1:
                ...
                if np.sign(np.cross(ve,tangets[0][0])) != np.sign(np.cross(ve,tangets[0][2])):
                    ...
                    vo = ve_mag * obs/np.linalg.norm(obs) * weight
                    if uav.debug == 1:
                        self.gp.append(ind1)
                        self.gp.append((obs,))
                        self.gp.append((vo,))
...
class FlockingModel:
    ...
    def __init__(self, R1_comm = 20.0, Rlim_1 = 2.0 , R_desire = 10.0, Kf = 0.1, Kc = 100_000.0 , Ka_vn = 0.1, Kfoc = 1.0, Ka_he = 30.0 , Kv_e = 10.0 ):
    ...
    def calculate_acc(self, uav: UAV, uavs: List[UAV], weights: Dict[int,float], ve: np.ndarray, he: float, model:ObstacleAvoidanceModel):
        ...
        for idx,o_uav in enumerate(uavs):
            ...
            if dist <= self.Rcomm_1:
                ...
                if dist<self.R_desire and len(model.get_A0(o_uav,system.obstacles,ve)[0]) == 1:
...
class FlockingControlAlgorithm:
    ...
    def __init__(self, num_pigeons, Tmax = 49.5, dt=0.5):
        ...
        if self.enable_render:
            ...
            shutil.rmtree("plt/3");os.mkdir("plt/3")
            shutil.rmtree("plt/4");os.mkdir("plt/4")
            shutil.rmtree("plt/5");os.mkdir("plt/5")
