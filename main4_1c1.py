...
class FlockingModel:
    ...
    def calculate_acc(self, uav: UAV, uavs: List[UAV], weights: Dict[int,float], ve: np.ndarray, he: float):
        ...
        if np.abs(xy_force[0])+np.abs(xy_force[1])>100:
            print(xy_force,force_f,force_a_vn,force_c)
...
class ObstacleAvoidanceModel:
    ...
    def get_near_tangent(self,yaw1:Tuple[float] , yaw2:Tuple[float]):
        l1,m1,r1 = yaw1
        l2,m2,r2 = yaw2
    ...
    def calculate_force(self, uav: UAV, obstacles: List[Obstacle], ve: np.ndarray, weight:int):
        ...
        print([obstacles.index(x) for x in attention])
        ...
        if len(attention) == 0:
            ...
            if len(attention) == 1:
                ...
                if yaws[0][2] < uav.psi < yaws[0][0] or yaws[0][0] < uav.psi < yaws[0][2]:
                    obs = tangets[0][0] if yaws[0][1] > 0 else tangets[0][2]
...
class FlockingControlAlgorithm:
    ...
    def simulate(self):
        ...
        for t in np.arange(0,self.Tmax,self.dt):
            ...
            if int(t/self.dt) % 2 == 0: # Render every 2 steps
